...
export class ProductFormComponent implements OnInit {
  ...
  addVariant() {
    console.log(
      'ProductForm: addVariant clicked. Current variants count:',
      this.form.variants?.length,
    );
  ...
  removeVariant(index: number) {
    ...
    if (!this.form.variants[index]._id) {
      ...
      console.log(
        'ProductForm: Cannot remove existing variant with _id. Consider toggling isActive instead.',
      );
    }
  }
  ...
  onSave() {
    ...
    const skus = this.form.variants.map((v: any) => v.sku?.trim()).filter((sku: string) => !!sku);

    const hasDuplicateSkus = new Set(skus).size !== skus.length;
    if (hasDuplicateSkus) {
      this.toastService.error(
        'Erreur: Vous avez des SKUs en double dans vos variantes. Chaque SKU doit être unique.',
      );
      return;
    }
