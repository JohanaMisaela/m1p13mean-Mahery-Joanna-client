...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterLink, FontAwesomeModule, StarRatingComponent, ShopReportComponent],
...
export class ShopHeaderComponent implements OnChanges {
  ...
  prevImage() {
    if (!this.shop?.gallery?.length) return;
    const prev =
      (this.currentGalleryIndex() - 1 + this.shop.gallery.length) % this.shop.gallery.length;
